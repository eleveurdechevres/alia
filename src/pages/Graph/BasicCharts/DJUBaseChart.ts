...
export class DJUBaseChart extends BaseChart {
    ...
    private updatetimeDomain() {
        const time_start = this.datum.reduce((end: number, data: IDJUData) => data.dateEnd.getDate() > end ? data.dateEnd : end, Number.NEGATIVE_INFINITY);
        const time_end = this.datum.reduce((start: number, data: IDJUData) => data.dateBegin.getDate() < start ? data.dateBegin : start, Number.POSITIVE_INFINITY);
    ...
    private updateYDomain() {
        ...
        this.yMin = 0;
        // this.yMax = 10;
        this.yMax = d3.max(this.datum, (d: IDJUData, i: number) => d.DJU);
    ...
    public updateRectanglesZoom = () => {
        ...
        .transition()
        .attr('x', (d: IDJUData) => this.timeScaleChart(d.dateBegin))
        .attr('width', (data: IDJUData) => {
            return this.timeScaleChart(data.dateEnd) - this.timeScaleChart(data.dateBegin)
        })
        .attr('y', (d: IDJUData) => this.yChart(d.DJU))
    ...
    public updateRectangles = () => {
        ...
        rectangles.enter()
            .append('rect')
            .attr('class', 'dju_rect')
            .attr('fill', (d: IDJUData) => d.dateBegin.getDay() === 0 ? 'Steelblue' : 'LightGoldenRodYellow')
            .attr('stroke', 'gray')
